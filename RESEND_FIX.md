...
Resend 使用 `onboarding@resend.dev` 作为发件人时，只能发送邮件到注册账户的邮箱（maverik.he@gmail.com），无法直接发送到其他邮箱（如 mingjie.he@timontech.cn）。
...
3. 邮件内容包含：实际收件人 `mingjie.he@timontech.cn`
4. 可以手动转发或设置自动转发规则
...
如果需要自动转发到 `mingjie.he@timontech.cn`：

1. 登录 Gmail（maverik.he@gmail.com）
2. 设置 > 转发和 POP/IMAP
3. 添加转发地址：mingjie.he@timontech.cn
4. 验证转发地址
5. 启用自动转发
